...
"""Phase 0 + dynamic obstacles variant. Same Nav2/MPPI/sensor stack as
tb3_mppi_sim.launch.py, only the world changes (5 capsule pedestrians driven
by gz-sim-trajectory-follower-system in tb3_sandbox_pedestrians.sdf.xacro).
"""
...
def generate_launch_description():
    ...
    default_world = os.path.join(
        pkg_share, 'worlds', 'tb3_sandbox_pedestrians.sdf.xacro')
    ...
    declare_world = DeclareLaunchArgument(
        ...
        description='SDF/xacro world. Defaults to the pedestrian variant.')
    declare_map = DeclareLaunchArgument(
        'map', default_value=default_map,
        description='Same static map as the baseline; only dynamic actors differ.')
