...
- **측정된 숫자 (02:00 sidecar, 재사용 — suite 비용 0). 첫 판독은 틀렸고 receipt 전에 잡았다**: 처음엔 526 node 전부를 합쳐 3837.2 s / meta-guard 59.3% 로 읽었다. 그 합은 suite total 이 아니다 — `nproc` 은 16 이지만 **xdist 가 설치되어 있지 않다**. 14 개 block 은 worker 가 아니라 meta-guard test 가 띄운 **중첩 pytest run** 이고, 각자 잘린 top-40 을 찍는다. block 간 합산은 이중 계산이며 (inner run 시간은 outer test 의 duration 에 *포함*된다) 어느 block 도 완전한 census 가 아니다.
- **바르게 읽으면**: block 10 이 outer session 이고 그 top-40 합이 **1280.5 s** — receipt 의 1281.95 s wall 과 일치한다. 즉 suite 전체가 사실상 test 40 개다. 그 안에서 **meta-guard 22 개 = 1248.1 s = 97.5%**, 그리고 한 파일로 좁혀진다 — **`test_exemption_masking` 16 개 = 1133.5 s = 전체 suite 의 88.4%** (그중 6 개가 165–242 s). meta-guard 가 아닌 최대 test 는 **12.5 s**. rollout test 는 근사적으로 **공짜**다.
