## Q-205 — 2026-08-26 — `[meta]` `@pytest.mark.slow` 는 **어느 집합**에 붙는가: 측정된 비용 집합인가, 유도된 rollout 집합인가? 둘은 거의 서로소다
...
- **측정된 숫자 (02:00 sidecar, 재사용 — suite 비용 0)**: test time 합계 **3837.2 s** (14 개 worker `--durations` block, `-j16` 에서 wall 1282 s). ≥1 s 인 249 개가 3754.7 s. 그중 **meta-guard 68 개가 2227.8 s — 전체 측정 test time 의 59.3%**. suite 비용을 지배하는 것은 MPPI 가 아니라 프로젝트가 자기 자신을 검사하는 기계다.
- **Trade-off**: (a) **측정 비용 집합에 marker** — wall clock 을 실제로 되사온다 (59% 가 north star 와 무관한 guard 에서 나온다). 그러나 marker 이름 `slow` 가 "rollout" 을 뜻한다는 서사가 깨지고, guard 를 slow lane 에 넣으면 그 guard 들이 매 cycle 돌지 않게 된다 — D-044 의 "치울 수 없는 check 는 mute 된다" 가 정확히 이 모양이다. vs (b) **유도된 rollout 집합에 marker** — 이름과 의미가 맞고 guard 는 계속 매번 돈다. 그러나 84 개 중 대부분이 싸므로 wall clock 이 거의 안 줄고, 그러면 8-controller install 은 여전히 한 cycle 에 안 들어간다 — marker 를 붙이는 이유 자체가 사라진다. vs (c) **marker 두 개** — `slow` (비용) 와 `rollout` (의미) 을 분리하고 install cycle 은 `-m "not slow"`, census cycle 은 `-m rollout` 을 쓴다. 정직하지만 marker 축이 하나 더 늘고 두 census 를 다 유지해야 한다.
- **Lean**: **(c)**, 잠정. (a) 와 (b) 는 각각 상대가 가진 것을 잃는데, 둘이 거의 서로소라는 것은 애초에 **한 축으로 두 질문에 답하려 했다**는 뜻이다. 다만 (c) 의 진짜 비용은 미측정 — meta-guard 68 개를 slow lane 에 넣었을 때 그것들이 실제로 얼마나 자주 돌게 되는지, 그리고 D-044 의 mute 가 발생하는지는 세어본 적이 없다.
- **다음 action**: 다음 cycle, rollout 0. 먼저 `-m "not slow"` 가 (a) 집합에서 실제로 얼마를 되사오는지 sidecar 로 계산 (초 단위 — 이미 있는 log 로 산술만 하면 된다). 그 숫자가 install 을 한 cycle 에 넣기에 충분하지 않으면 (c) 도 (a) 도 의미가 없고 Q 는 파일 분할 (Q-203 (c)) 로 되돌아간다.
