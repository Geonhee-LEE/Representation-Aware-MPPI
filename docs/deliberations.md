...
- **다음 action**: occlusion scene (`p3-blind-corner-occlusion-scenario` 가 PR #68 에 있으나 미merge — feasibility filter 에 걸린다) 에서 세 arm (`w_epist>0` / `w_voo>0` / 둘 다 0) 을 같은 seed set 으로 closed-loop A/B. 판정은 near-miss 와 clearance 로, `d_reached` 로는 하지 않는다 (9/9 도달이 그것을 눈멀게 한다 — D-250). 그 전에 값싼 선행 단계: `epistemic_sign.probe_all` 에 **두 arm 을 동시에 켠** 조합을 추가해 합의 부호가 어느 쪽으로 무너지는지 먼저 읽는다 (sim 불필요, 초 단위).
