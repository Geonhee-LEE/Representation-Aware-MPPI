...
- **Trade-off**: (a) keep typing entries as they cost suites — cheap per instance, and the record shows ~9 instances so far, each priced at one red receipt (~740 s); versus (b) find a derivation — one-time cost, unknown size, and the module docstring already argues no AST signature separates a census from any other derived collection.
- **Lean**: (a) for now, but stop treating each new entry as a one-off. The nine entries have a shape worth reading as data: every one is a population **pinned by an `==` against a literal in a test**. That is a candidate signature — `assert <derivation> == <literal>` where the left side calls into `eval/mppi_sandbox/` — and it is testable against the known nine before anything is shipped, exactly as D-480 tested `REGISTRIES` against the known two.
- **다음 action**: whoever next pays a red suite for an unlisted census — derive the `assert <call> == <literal>` population across `eval/mppi_sandbox/tests/`, and check it against the nine entries in `CENSUSES` plus the five in `UNCOVERED` before writing any code. If it recovers ≥ 12 of 14, it is worth shipping; if it recovers the same 0/2 `REGISTRIES` did, close Q-183 and Q-207 as structural and stop asking.
...
- **왜 어긋났는가 (Q-203 의 예측과 다르다)**: Q-203 의 lean 은 어긋남 = **간접 호출**이라고 예측했다. 실제 원인은 다르다 — `measured_rollout_tests` 는 **duration ≥ 1.0 s** 로 threshold 한다. 이것은 *비용* oracle 이지 *rollout* oracle 이 아니다. 측정 상위는 rollout 이 아예 아니다: `test_guard_reflexivity::test_report_names_its_own_findings` **304.40 s**, `test_exemption_masking` 5 개가 167–242 s. 즉 near-disjointness 는 walk 만 기소하는 게 아니라 proxy 를 기소한다.
- **⚠️ 판독 정정 (세 번째가 맞다)**: (1) 526 node 를 전부 합쳐 3837.2 s / 59.3% → 틀림. (2) `nproc`=16 이지만 xdist 미설치이므로 14 block 은 **중첩 run**, 합산은 이중 계산 → **이것도 틀림**, 그리고 이 오답이 commit `dff853e` 에 실려 push 됐다. (3) receipt 요약줄이 처음부터 답을 찍고 있었다 — **`across 14 shards`**. `push_preflight.record_sharded` 가 suite 를 file 단위로 쪼개 **동시 실행**한다. worker 도 중첩도 아니고 shard 이며, block 은 서로소다. 이중 계산은 없다.
- **측정된 숫자 (receipt 의 `shard_seconds` — 재측정 불필요)**: 14 shard, CPU 합계 **4044.9 s**, wall = **1274.7 s** = 가장 느린 shard. `push_preflight` docstring 이 규칙을 직접 적어놨다 — *"the suite's wall clock **is** its slowest shard"*.
- **따라서 발견은 "비중" 이 아니라 load imbalance 이고, 더 나쁘다**: 완벽히 균형 잡히면 4044.9/14 = **288.9 s**. 실제 1274.7 s — 자기 자신의 균형 하한의 **4.4배**. shard 10 이 1274.7 s, 두 번째가 609.2 s, 가장 빠른 것이 51.1 s.
- **그리고 shard 10 의 88.9% 가 한 파일이다**: `test_exemption_masking` 16 개 = 1133.5 s (shard 10 의 1274.7 s 중), 그중 6 개가 165–242 s. meta-guard 가 아닌 최대 test 는 **12.5 s** — rollout test 는 근사적으로 **공짜**다.
- **sharder 로는 못 고친다. 이유가 그 source 에 적혀 있다**: `suite_shard.file_weight` 는 **file 크기(byte)** 이고 runtime 과 아무 관계가 없다. 게다가 split 이 *file* 단위라 나눌 수 없는 1133.5 s 파일이 **hard floor** 를 만든다 — 어떤 file-level shard 배치도 wall 을 1133.5 s (지금의 89%) 아래로 못 내린다. rebalancing 은 lever 가 아니다. 그 파일이 lever 다.
...
- **진행 (2026-08-26 02:00, D-473)**: (b) 는 **구현됐다** — `eval/mppi_sandbox/lam_rollout.py`, 20줄이 아니라 383줄이 들었고 그 차액의 대부분은 keying 이다 (bare-name graph 는 4233 중 2428 을 rollout 이라고 했다; `module.func` 로 한정하니 **84**). (a) 도 이번 receipt 에 `--durations=40` 을 붙여 **비용 0 으로** 확보했고, sidecar 는 `/tmp/suite-receipt.json.log`. 남은 것은 `compare` 실행 하나뿐이며 suite 는 이미 값을 치렀다.
- **다음 action**: 다음 cycle 이 `python3 -m eval.mppi_sandbox.lam_rollout compare /tmp/suite-receipt.json.log` 를 돌린다 (초 단위, suite 없음). `measured_only` 가 이 Q 의 답이다 — walk 이 놓친 **간접 호출**. 그 집합에 `@pytest.mark.slow` 를 붙이면 `-m "not slow"` 가 처음으로 table assertion 과 rollout assertion 을 분리하고, 그때 install 이 한 cycle 에 들어간다. **왜 이번 cycle 이 아닌가**: sidecar 는 receipt 가 끝나야 존재하고 D-315 는 receipt 이후 tracked write 를 금지한다 — 두 규칙의 교차가 자연 경계다.
