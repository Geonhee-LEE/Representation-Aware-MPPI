...
def to_yaml(cals: Sequence[SceneCalibration], ladder: Sequence[float]) -> str:
    ...
    return _render(
        {"calibration_weight": weight, "ladder": tuple(ladder),
         "seeds": len(list(ab.DEFAULT_SEEDS)), "band_width": band_width()},
        [_cell_mapping(c) for c in cals])


def _cell_mapping(cal: SceneCalibration) -> dict:
    """The cell as the table records it — the projection `_render` writes and
    `load_windows` reads back. Having exactly one of these is what lets a
    merged table and a generated one be the same file format rather than two
    formats that agree today (D-047)."""
    return {
        "scenario": cal.scenario,
        "controller": cal.controller,
        "admissible": tuple(cal.admissible),
        "ladder": tuple(cal.ladder),
        "min_spread": cal.min_spread,
        "completes_anywhere": cal.completes_anywhere,
        "calibratable": cal.is_calibratable,
    }


def _render(header: dict, cells: Sequence[dict]) -> str:
    """Emit a table from a header and already-projected cells.

    Split out of `to_yaml` so `merge_tables` can re-emit a table it did not
    generate. The header is passed in rather than re-derived from the
    environment: re-stamping `seeds`/`band_width` from *this* process onto
    cells measured by another one is the same false provenance D-107 booked,
    one level up from the per-cell weight `to_yaml` already refuses.
    """
    lines = [
        ...
        f"#   the band width ({header['band_width']:.0f}x) no rung can qualify, so an",
        ...
        "",
        f"calibration_weight: {header['calibration_weight']:g}",
        f"ladder: [{', '.join(str(x) for x in header['ladder'])}]",
        f"seeds: {header['seeds']}",
        f"band_width: {header['band_width']:.1f}",
        "",
        "cells:",
    ]
    for cell in cells:
        lines += [
            ...
            f"    controller: {cell['controller']}",
            f"    admissible: [{', '.join(str(x) for x in cell['admissible'])}]",
            f"    ladder: [{', '.join(str(x) for x in cell['ladder'])}]",
            f"    min_spread: {cell['min_spread']:.2f}",
            f"    completes_anywhere: {str(cell['completes_anywhere']).lower()}",
            f"    calibratable: {str(cell['calibratable']).lower()}",
        ]
    shared: tuple[float, ...] = ()
    if cells:
        acc = set(cells[0]["admissible"])
        for cell in cells[1:]:
            acc &= set(cell["admissible"])
        shared = tuple(sorted(acc))
    lines += [
        "",
...
def load_header(path: str) -> dict:
    """The table's top-level record: weight, ladder, seed count, band width.

    `load_windows` deliberately returns only cells, which is all any reader
    needed while tables were whole-file regenerations. A merge has to compare
    the two files' headers before it may join their cells, so the header needs
    a reader of its own.
    """
    header: dict = {}
    with open(path) as fh:
        for raw in fh:
            ...
            if key == "calibration_weight":
                header["calibration_weight"] = float(val)
            elif key == "ladder":
                header["ladder"] = tuple(
                    float(x) for x in val.strip("[]").split(",") if x.strip())
            elif key == "seeds":
                header["seeds"] = int(val)
            elif key == "band_width":
                header["band_width"] = float(val)
    return header


#: A merge whose two tables disagree on the weight they were walked at. One
#: table records one `calibration_weight`, so joining these cells would put a
#: window under a weight it was never measured at — the file-level form of the
#: per-cell refusal `to_yaml` already makes.
WEIGHT_MISMATCH = "WEIGHT_MISMATCH"

#: The tables agree on weight but not on ladder, seed count, or band width.
#: These are the terms the windows *mean*: `admissible` is a conjunction over
#: seeds and a selection from the ladder, so cells from two ladders under one
#: header would be graded by a caller as if they had been asked the same
#: question.
PROTOCOL_MISMATCH = "PROTOCOL_MISMATCH"

#: A `(scenario, controller)` cell appears in both tables. Refused rather than
#: resolved: silently keeping either one turns a re-measurement into an edit
#: nobody recorded, and D-142 measured that the same cell genuinely moves.
DUPLICATE_CELL = "DUPLICATE_CELL"


class MergeRefused(ValueError):
    """A merge that would have produced a table making a claim neither input
    measured. Carries the verdict name so callers can branch on the reason."""
...
def merge_tables(base_path: str, extra_path: str) -> str:
    """Render one table holding the cells of both, or refuse.

    Adding a controller column to an existing table is otherwise a choice
    between two bad options: re-walk the whole matrix (D-141 measured that the
    existing 16 cells reproduce exactly, so the re-walk is pure cost), or
    hand-edit a file whose own header says not to. This is the third: the new
    column is *measured* by a normal `calibrate_lam` run into its own file, and
    the join is a mechanical operation with named refusals.

    The header is taken from `base_path` and both files must agree on it, so
    the merged table records the protocol both columns were actually walked
    under rather than the merging process's environment.
    """
    base_header, extra_header = load_header(base_path), load_header(extra_path)
    if base_header.get("calibration_weight") != \
            extra_header.get("calibration_weight"):
        raise MergeRefused(
            WEIGHT_MISMATCH,
            f"{base_path} walked at w="
            f"{base_header.get('calibration_weight')}, {extra_path} at w="
            f"{extra_header.get('calibration_weight')}")
    for field_name in ("ladder", "seeds", "band_width"):
        if base_header.get(field_name) != extra_header.get(field_name):
            raise MergeRefused(
                PROTOCOL_MISMATCH,
                f"{field_name} differs: {base_header.get(field_name)} != "
                f"{extra_header.get(field_name)}")
    base_cells, extra_cells = load_windows(base_path), load_windows(extra_path)
    clash = sorted(set(base_cells) & set(extra_cells))
    if clash:
        raise MergeRefused(
            DUPLICATE_CELL,
            f"{len(clash)} cell(s) in both tables, e.g. {clash[0]} — "
            f"re-measuring a cell is a new table, not a merge")
    merged = list(base_cells.values()) + list(extra_cells.values())
    return _render(base_header, merged)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ap.add_argument("--w-obs-soft", type=float, default=None,
                    help="obstacle weight to walk every ladder at; recorded "
                         ...
                         "per weight via --out.")
    ap.add_argument("--merge", nargs=2, metavar=("BASE", "EXTRA"),
                    help="join two tables walked at the same weight and "
                         "protocol into --out and exit, calibrating nothing. "
                         "How a controller column is added to an existing "
                         "table without re-walking the cells D-141 showed "
                         "reproduce exactly.")
    args = ap.parse_args(argv)

    if args.merge:
        base, extra = args.merge
        try:
            text = merge_tables(base, extra)
        except MergeRefused as exc:
            print(f"merge refused — {exc}")
            return 1
        with open(args.out, "w") as fh:
            fh.write(text)
        print(f"merged {base} + {extra} -> {args.out} "
              f"({len(load_windows(args.out))} cells)")
        return 0
