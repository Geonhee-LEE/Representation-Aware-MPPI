...
def test_and_shaped_guards_are_exactly_these_four(pool):
    ...
    assert len(pool) == 120, (
        ...
        "D-330 left this at **120** on purpose, and the near-miss is the entry worth keeping: `scene_transfer.format_grade` *did* enter — not a guard anyone wrote as a guard, but a **printer** whose one-line `if arm in REPRESENTATION_ARMS` tagged its output rows, which is a population filtered by a named constant and so indistinguishable from an exemption. The module's actual predicates (`any_arm_generalises`, `arms_that_generalise`, `winners`) never entered, because each filters by a *computed* property. So the detector keys on the exemption's shape, not the author's intent, and a cosmetic label was about to register a **category** constant as a watched allow-list in four registries (`exemption_control`, `exemption_masking`, `guard_direction`, and this file's own unwatched list). The cheap repair is not to bump six pins: it is to delete the label, which is what happened. Cost of finding that out the slow way: one full red suite, because `census_preempt` names these four registries in its own `UNCOVERED` line and this cycle read the line without acting on it.")