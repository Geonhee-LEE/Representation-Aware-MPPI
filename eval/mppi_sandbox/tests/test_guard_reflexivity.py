...
def test_mirrors_are_the_known_eight(pool):
...
def test_the_shallow_predicate_was_hiding_two_more_guards():
    ...
    **after** the fix would have been half-invisible **before** it.
    ...
    assert deep - shallow == {
        ...
        # D-105: `unsupported` filters against `finding_grades()`, a same-module
        # call, so the shallow scan cannot see it — D-051's reason, and the
        # first time it applies to a module's *headline* function.
