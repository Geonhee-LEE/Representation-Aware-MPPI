...
def test_the_shallow_predicate_was_hiding_two_more_guards():
    ...
    **after** the fix would have been half-invisible **before** it.
    ...
    assert deep - shallow == {
        ...
        # D-334: `informative_separators` excludes the observables that never
        # move across seeds, filtering against `set(constant_observables())` —
        # a same-module call bound to a local before use, which is
        # `window_axis_reach.consumers`' exact shape and stops the shallow scan
        # at the call for D-051's reason.  This entry was written **before** the
        # suite rather than after it: D-333 landed in this same literal by
        # paying a 824 s red first, and its note above says the pre-empt cannot
        # see placement.  It still cannot; what changed is that the note was
        # read.
        "scene_separability.informative_separators",
    }
