...
def test_shipped_table_is_still_unkeyed():
    """Keying `lam_windows.yaml` means *re-running* it, not editing its header.
    This test fails the day someone stamps the file without measuring — which
    is the cheap, wrong way to make `UNKEYED` go away, and would give ~24
    resolved cells a provenance nobody re-derived (D-107).

    Delete this test in the same commit that regenerates the table.
    """
    if not os.path.exists(SHIPPED):
        pytest.skip(f"{SHIPPED} not generated")
    _, weight = lwk._rows(SHIPPED)
    assert weight is None, (
        f"{SHIPPED} now records calibration_weight={weight} — if it was "
        f"regenerated by `calibrate_lam --w-obs-soft`, delete this test; if it "
        f"was hand-edited, revert it")
