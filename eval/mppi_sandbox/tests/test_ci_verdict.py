...
``ci_verdict`` are a measurement of one run and will be superseded by the next.
...
from eval.mppi_sandbox import ci_verdict
...
def test_the_snapshot_covers_exactly_the_shards_the_workflow_declares():
    ...
    """
    declared = ci_verdict.shards_declared_by_workflow()
    snapshot_shards = {
        int(name.rsplit("(", 1)[1].rstrip(")"))
        for name, _, _ in ci_verdict.RUN_32756918395
...
def test_failing_tests_refuses_on_a_run_with_no_verdict():
    ...
    with pytest.raises(ci_verdict.IncompleteRun) as excinfo:
        ci_verdict.failing_tests()
...
def test_the_floor_reader_always_says_it_is_a_floor():
    floor = ci_verdict.failure_floor()
    assert floor["is_floor"] is True
    assert floor["count"] == len(ci_verdict.OBSERVED_FAILURES)
    assert floor["unverdicted_jobs"], "a floor with nothing missing is not a floor"
    assert "CI_PARTIAL" in ci_verdict.reading()
    assert ">= 7" in ci_verdict.reading()
...
def test_a_cancelled_job_is_not_a_passing_one():
    ...
    """
    missing = dict(ci_verdict.unverdicted())
    ...
    complete = tuple(
        ...
        for name, conclusion, seconds in ci_verdict.RUN_32756918395
    )
    assert ci_verdict.is_complete(complete)
    assert ci_verdict.failure_floor(complete)["is_floor"] is False
...
def test_state_undercounted_and_the_gap_is_a_strict_subset():
    ...
    taxonomy.
    """
    observed = set(ci_verdict.OBSERVED_FAILURES)
    state = set(ci_verdict.STATE_READING)
...
def test_shard_six_breached_the_declared_ceiling():
    ...
    breaches = ci_verdict.ceiling_breaches()
    assert breaches == ("pytest (fast) (6)",)
    # And it is not a breach against a ceiling we invented here.
    assert "timeout-minutes: 30" in ci_verdict._WORKFLOW.read_text(encoding="utf-8")
