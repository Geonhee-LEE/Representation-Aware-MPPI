...
class TestShardedPriceHasNoAdmissibleEndHere:
    ...
    def test_registry_is_serial_only(self):
        """The sharded price is absent, and its absence is the decision.

        Written as an exact-membership check rather than a bound so that a future
        append has to come here and read the reason, instead of passing because
        it happened to land inside a range.
        """
        assert 488 not in [s for s, _ in cw.OBSERVED_SUITE_SECONDS]
        assert [s for s, _ in cw.OBSERVED_SUITE_SECONDS] == [717, 1091, 1223]

    def test_ceiling_prices_an_unknown_mode_serially(self):
        """``SUITE_SECONDS`` is read only when no receipt exists.
    ...
    def test_floor_grades_a_population_that_predates_sharding(self):
        ...
        assert cw.PREMATURE_SUITE_SECONDS == 717
