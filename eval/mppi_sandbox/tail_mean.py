...
* **One scene.** :data:`SCENE` only. `city_curved_v0`, the unexcited endpoint,
  is unharvested for TVaR. D-372's finding was that the dividing line is the
  **column**, not the scene, and D-371 was wrong in exactly the direction of
  generalising one scene to five — so this licenses `cafe_convoy_v0` and
  nothing else until the second endpoint is harvested (118 s, :func:`retake`).
